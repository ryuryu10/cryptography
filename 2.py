...
def main():
    key = 'RYUSSBLOG'
    msg = 'WELCOMETOMYBLOG'
    print('____원본 메시지 :\t%s' %msg.upper())

    cipertext = transposition(msg, key, ENC)
    print('암호화된 메시지 : \t%s' %cipertext)

    deciphertext = transposition(cipertext, key, DEC)
    print('복호화된 메시지 : \t%s' %deciphertext)
main()